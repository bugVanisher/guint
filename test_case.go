...
func (this *testCase) Prepare(setup, teardown int, outerFixture reflect.Value, pkgName string, skipChannel chan bool) {
	this.setup = setup
...
func (this *testCase) initializeFixture(innerT *testing.T) {
	this.innerFixture = newFixture(innerT, testing.Verbose(), this.testPackageName, this.skipChannel)
	if this.parallel {
